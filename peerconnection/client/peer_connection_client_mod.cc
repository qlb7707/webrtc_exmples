...
//??This function seems useless for websocket
...
//register callback
//replace control_socket_ & hang_get_
...
//override DoConnect to send websocket
void PeerConnectionClient::DoConnect()
{
}

//override SendToPeer to send websocket
...
//replace my_id_,control_socket_
//if hanging_get the same type as control_socket_
//if so, then replace hanging_get_
...
//This function read data from socket to data and check if should close
//??Think about what to do if we use websocket
...
//This function parse data from socket to obtain control_data & content_length
//Then parse peer_id & end of head
//If it is the first response get the info of connected peers  
...
//This function parse data from socket to obtain notification_data_
//then parse peer_id
...
//Get the response status for example 200
...
//parse reponse to obtain peer_id & eoh
...
//replace the socket & hanging_get_
